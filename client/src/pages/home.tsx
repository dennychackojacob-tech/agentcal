...
export default function Home() {
  ...
  const { data: schedule, isLoading: scheduleLoading } = useQuery<DailySchedule>({
    queryKey: ['/api/schedule', agentId, today],
    enabled: !!agentId,
  });
  ...
  const optimizeRouteMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await apiRequest("POST", `/api/optimize-route`, { agentId, date: today });
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/schedule', agentId, today] });
    }
  });
